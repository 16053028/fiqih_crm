...
<?php echo form_open(base_url('master_project/subscription/' . $row->id_pelanggan)); ?>
  ...
  <div class="text-danger"><?php echo form_error('inputLayanan'); ?></div>
    ...
    <select class="form-select" aria-label="Default select" id="inputLayanan" name="inputLayanan">
      <option disabled>- Select one -</option>
      <?php
        foreach ($layanans as $layanan) { ?>
          <option value="<?= $layanan->id_layanan ?>" <?= ($layanan->id_layanan == $row->id_layanan) ? "selected" : "" ; ?>><?= $layanan->nama_layanan ?></option>
        <?php } ?>
    </select>
  </div>
